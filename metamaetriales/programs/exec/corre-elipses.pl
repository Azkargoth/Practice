#!/usr/bin/env perl
#######################
# makes movies of eta and corresponding elipses
use strict;
use Pod::Usage;
use Getopt::Long;
use File::Basename;
...
corre-elipses.pl --od=s --scale=f --angle=f --Nh=i --epsa=s --epsb=s --axes=s [--nem=s] --ep=s
...
if ("$axes"eq"principal") {
  ...
  if ("$nem"eq"vsw"){
    # name for movie
    system "ls plots/elipse*$angle*$scale*$Nh*epsa*$epsa*epsb*$epsb*.pdf | sort -tW -n -k 2 > lili1";
    system "ls plots/aa*$angle*$scale*$Nh*epsa*$epsa*epsb*$epsb*.pdf | sort -tW -n -k 2 > lili2";
  }
}
...
\\includegraphics[scale=.8]{rem3}\\\\
\\includegraphics[scale=.8]{rem2}\\\\
\\hspace{1.1cm}\\includegraphics[scale=.575]{rem1}%
...
system "cp lrem-$n.pdf keep/elipse-lrem-$n.pdf" if $keep;
system "rm rem*";
}
...
system "pdflatex anima.tex > log";
system "mv anima.pdf $od/elipse-m-$name.pdf";
print  "\n\tMovie: $od/elipse-m-$name.pdf\n";
printf "\n";
